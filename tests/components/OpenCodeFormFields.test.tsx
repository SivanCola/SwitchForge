...
describe("OpenCodeFormFields", () => {
  it("surfaces existing provider headers", () => {
    renderOpenCodeForm({
      headers: {
        "HTTP-Referer": "https://switchforge.app",
        "X-Title": "SwitchForge",
      },
    });
    ...
    expect(
      screen.getByDisplayValue("https://switchforge.app"),
    ).toBeInTheDocument();
    expect(screen.getByDisplayValue("X-Title")).toBeInTheDocument();
    expect(screen.getByDisplayValue("SwitchForge")).toBeInTheDocument();
  });

  it("updates provider headers", () => {
    const onHeadersChange = vi.fn();
    renderOpenCodeForm({
      headers: { "X-Title": "SwitchForge" },
      onHeadersChange,
    });

    fireEvent.change(screen.getByDisplayValue("SwitchForge"), {
