...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.switchforge/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（单文件输出到 <app_config_dir>/logs/switchforge.log）
            {
                ...
                let log_file_path = log_dir.join("switchforge.log");
                    ...
                    tauri_plugin_log::Builder::default()
                        ...
                        .targets([
                            ...
                            Target::new(TargetKind::Folder {
                                path: log_dir,
                                file_name: Some("switchforge".into()),
                            }),
                        ])
            ...
            let db_path = app_config_dir.join("switchforge.db");
            ...
            // 首次启动只读取 Claude Code 与 Codex 的 live 配置。其它客户端、
            // 公共 Skills 仓库和历史会话都不会被隐式探测。
            //
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 SwitchForge 的工作方式。
            ...
            // MCP 首次导入同样只覆盖 Claude Code 与 Codex。
            ...
            // SwitchForge only keeps local database backup maintenance running.
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older SwitchForge version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including switchforge.db)\n\
            2) If you see “database version is newer”, please upgrade SwitchForge\n\
...
mod tests {
    ...
    #[test]
    fn startup_does_not_schedule_removed_services() {
        ...
        let setup_source = source
            ...
            .expect("setup source");
        let forbidden_parts = [
            ["tauri_plugin_", "deep_link::init"],
            ["webdav_auto_sync", "::start_worker"],
            ["s3_auto_sync", "::start_worker"],
            ["restore_proxy_state", "_on_startup"],
            ["session_usage::", "sync_claude_session_logs"],
            ["refresh_all_usage", "_in_tray"],
            ["import_opencode_providers", "_from_live"],
            ["import_openclaw_providers", "_from_live"],
            ["import_hermes_providers", "_from_live"],
        ];

        for parts in forbidden_parts {
            let forbidden = parts.concat();
            assert!(
                !setup_source.contains(&forbidden),
                "removed startup behavior reintroduced: {forbidden}"
            );
        }
    }
