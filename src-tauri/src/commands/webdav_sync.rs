...
mod tests {
    ...
    fn persist_sync_error_updates_status_without_overwriting_credentials() {
        let test_home = std::env::temp_dir().join("switchforge-sync-error-status-test");
    ...
    fn require_enabled_webdav_settings_rejects_disabled_config() {
        let test_home = std::env::temp_dir().join("switchforge-sync-enabled-disabled-test");
        let _ = std::fs::remove_dir_all(&test_home);
        std::fs::create_dir_all(&test_home).expect("create test home");
        std::env::set_var("SWITCHFORGE_TEST_HOME", &test_home);
    ...
    fn require_enabled_webdav_settings_returns_settings_when_enabled() {
        let test_home = std::env::temp_dir().join("switchforge-sync-enabled-ok-test");
        let _ = std::fs::remove_dir_all(&test_home);
        std::fs::create_dir_all(&test_home).expect("create test home");
        std::env::set_var("SWITCHFORGE_TEST_HOME", &test_home);
