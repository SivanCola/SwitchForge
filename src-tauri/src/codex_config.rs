...
pub const SWITCHFORGE_CODEX_MODEL_PROVIDER_ID: &str = "custom";
/// Temporary model-provider id used while the built-in `codex-official`
/// provider is routed through SwitchForge.  A dedicated id is an ownership
...
pub const SWITCHFORGE_CODEX_OFFICIAL_PROXY_PROVIDER_ID: &str = "switchforge-official";
pub const SWITCHFORGE_CODEX_MODEL_CATALOG_FILENAME: &str = "switchforge-model-catalog.json";
...
/// this per the vendors' official Codex docs. Also doubles as switchforge's
...
///
/// - `ProxyChat`: switchforge's proxy takes over and converts Responses<->Chat,
...
pub enum CodexCatalogToolProfile {
    ProxyChat,
    NativeResponses,
    /// Codex talks (through switchforge's proxy) to a native Anthropic Messages
...
pub fn get_codex_model_catalog_path() -> PathBuf {
    get_codex_config_dir().join(SWITCHFORGE_CODEX_MODEL_CATALOG_FILENAME)
}
...
fn set_codex_model_catalog_json_field(
    ...
    match catalog_path {
        Some(_) => {
            doc["model_catalog_json"] = toml_edit::value(SWITCHFORGE_CODEX_MODEL_CATALOG_FILENAME);
        }
        None => {
            let should_remove = doc
                ...
                .map(|path| {
                    Path::new(path).file_name().and_then(|name| name.to_str())
                        == Some(SWITCHFORGE_CODEX_MODEL_CATALOG_FILENAME)
                })
                .unwrap_or(false);
            if should_remove {
...
/// but only when it carries switchforge's own `"disabled"` sentinel, so switching
...
/// lifecycle is bound to the switchforge catalog pointer so the field is set/cleaned
...
/// switchforge–maintained catalog file referenced by `~/.codex/config.toml` and
...
/// switchforge–generated file (identified by filename
/// `switchforge-model-catalog.json`). A user-managed external catalog file is
...
/// Given `config.toml` text, resolve the on-disk path of the switchforge–owned
...
pub fn apply_codex_official_proxy_route(
    ...
    doc.as_table_mut().remove("experimental_bearer_token");
    doc["model_provider"] = toml_edit::value(SWITCHFORGE_CODEX_OFFICIAL_PROXY_PROVIDER_ID);
    ...
    // Clean only SwitchForge's placeholder from every stale provider table. Real
    ...
    providers.insert(
        SWITCHFORGE_CODEX_OFFICIAL_PROXY_PROVIDER_ID,
...
/// Whether a live Codex config is the official route projected by SwitchForge.
pub fn codex_config_has_official_proxy_route(config_text: &str) -> bool {
    if !config_text.contains(SWITCHFORGE_CODEX_OFFICIAL_PROXY_PROVIDER_ID) {
        return false;
    }
    config_text
        .parse::<DocumentMut>()
        ...
        .as_deref()
        == Some(SWITCHFORGE_CODEX_OFFICIAL_PROXY_PROVIDER_ID)
}

/// Remove only the official takeover route owned by SwitchForge. This is a
...
pub fn remove_codex_official_proxy_route(config_text: &str) -> Result<String, AppError> {
    ...
    if let Some(item) = doc.as_table_mut().remove("model_providers") {
        ...
        providers.remove(SWITCHFORGE_CODEX_OFFICIAL_PROXY_PROVIDER_ID);
        remove_codex_proxy_placeholders_from_providers(&mut providers);
...
pub fn inject_codex_unified_session_bucket(config_text: &str) -> Result<String, AppError> {
    ...
    let existing_custom_conflicts = doc
        .get("model_providers")
        .and_then(|item| item.as_table())
        .and_then(|providers| providers.get(SWITCHFORGE_CODEX_MODEL_PROVIDER_ID))
        .and_then(|item| item.as_table())
    ...
    doc["model_provider"] = toml_edit::value(SWITCHFORGE_CODEX_MODEL_PROVIDER_ID);
    ...
    if let Some(providers) = doc["model_providers"].as_table_mut() {
        if !providers.contains_key(SWITCHFORGE_CODEX_MODEL_PROVIDER_ID) {
            providers.insert(
                SWITCHFORGE_CODEX_MODEL_PROVIDER_ID,
...
pub fn strip_codex_unified_session_bucket(config_text: &str) -> Result<String, AppError> {
    ...
    if doc.get("model_provider").and_then(|item| item.as_str())
        != Some(SWITCHFORGE_CODEX_MODEL_PROVIDER_ID)
    {
        return Ok(config_text.to_string());
    }
    ...
    let providers_empty = doc["model_providers"]
        .as_table_mut()
        .map(|providers| {
            providers.remove(SWITCHFORGE_CODEX_MODEL_PROVIDER_ID);
            providers.is_empty()
        })
        .unwrap_or(false);
    if providers_empty {
...
mod tests {
    ...
    fn unified_session_bucket_injects_for_empty_official_config() {
        ...
        let custom = doc["model_providers"][SWITCHFORGE_CODEX_MODEL_PROVIDER_ID]
            .as_table()
...
"#;
        ...
        let provider = &doc["model_providers"][SWITCHFORGE_CODEX_OFFICIAL_PROXY_PROVIDER_ID];
        assert_eq!(
...
"#;
        ...
        assert!(projected_doc["model_providers"]
            .get(SWITCHFORGE_CODEX_OFFICIAL_PROXY_PROVIDER_ID)
            .is_some());
    ...
    fn unified_session_bucket_preserves_other_keys_and_explicit_routing() {
        let with_catalog = "model_catalog_json = \"switchforge-model-catalog.json\"\n";
...
name = "any"
"#;
        let catalog_path = Path::new("/tmp/switchforge-model-catalog.json");
...
"#;
    ...
    fn native_web_search_field_removes_own_sentinel_when_not_disabled() {
        ...
        // removing switchforge's own "disabled" sentinel.
...
"#;
        ...
        assert!(
            ...
            "switchforge's disabled sentinel should be removed when not native"
        );
    }
    ...
    fn native_web_search_field_preserves_user_value() {
        ...
        // only switchforge's "disabled" sentinel is owned/removable.
...
"#;
    ...
    fn resolve_catalog_path_returns_none_when_config_missing_field() {
        let generated = PathBuf::from("/tmp/.codex/switchforge-model-catalog.json");
    ...
    fn resolve_catalog_path_accepts_cc_switch_owned_file() {
        let generated = PathBuf::from("/tmp/.codex/switchforge-model-catalog.json");
        let config = r#"model_catalog_json = "/tmp/.codex/switchforge-model-catalog.json"
"#;
...
model = "glm-5"
"#;
        // Simulate a WSL UNC path as switchforge would see it on Windows;
        // the function now writes just the relative filename.
        let unc_path =
            Path::new(r"\\wsl.localhost\Ubuntu\home\user\.codex\switchforge-model-catalog.json");
        ...
        assert_eq!(
            written_path, SWITCHFORGE_CODEX_MODEL_CATALOG_FILENAME,
...
model = "glm-5"
"#;
        let regular_path = Path::new("/home/user/.codex/switchforge-model-catalog.json");
    ...
    fn set_catalog_json_none_removes_cc_switch_owned_by_filename() {
        ...
        let input = r#"model_catalog_json = "/home/user/.codex/switchforge-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove switchforge-owned field regardless of path format"
        );
    }
...
model_catalog_json = "switchforge-model-catalog.json"
"#;
        let generated_path = PathBuf::from("/home/user/.codex/switchforge-model-catalog.json");
...
"#;
        ...
        assert_eq!(
            result, None,
            "user-owned catalog should not be claimed by switchforge"
        );
    }

    #[test]
    fn set_catalog_json_none_removes_relative_path() {
        let input = r#"model_catalog_json = "switchforge-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove relative switchforge-owned field"
        );
    }
}
