...
/// SwitchForge 代理服务器当前监听的端口
static SWITCHFORGE_PROXY_PORT: OnceCell<RwLock<u16>> = OnceCell::new();

/// 设置 SwitchForge 代理服务器的监听端口
///
/// 应在代理服务器启动时调用，以便系统代理检测能正确识别自己的端口
pub fn set_proxy_port(port: u16) {
    if let Some(lock) = SWITCHFORGE_PROXY_PORT.get() {
        if let Ok(mut current_port) = lock.write() {
            *current_port = port;
            log::debug!("[GlobalProxy] Updated SwitchForge proxy port to {port}");
        }
    } else {
        let _ = SWITCHFORGE_PROXY_PORT.set(RwLock::new(port));
        log::debug!("[GlobalProxy] Initialized SwitchForge proxy port to {port}");
    }
}

/// 获取 SwitchForge 代理服务器的监听端口
fn get_proxy_port() -> u16 {
    SWITCHFORGE_PROXY_PORT
        .get()
...
fn proxy_points_to_loopback(value: &str) -> bool {
    ...
    if let Ok(parsed) = url::Url::parse(value) {
        if let Some(host) = parsed.host_str() {
            // 只有当主机是 loopback 且端口是 SwitchForge 的端口时才返回 true
...
mod tests {
    ...
    fn test_proxy_points_to_loopback() {
        // 设置 SwitchForge 代理端口为 15721（默认值）
        set_proxy_port(15721);

        // 只有指向 SwitchForge 自己端口的 loopback 地址才返回 true
