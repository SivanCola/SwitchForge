...
use crate::codex_config::{
    get_codex_config_dir, read_codex_config_text, SWITCHFORGE_CODEX_MODEL_PROVIDER_ID,
};
...
const LEGACY_SWITCHFORGE_CODEX_MODEL_PROVIDER_ID: &str = "switchforge";
...
const SWITCHFORGE_LEGACY_CODEX_MODEL_PROVIDER_IDS: &[&str] = &[
    LEGACY_SWITCHFORGE_CODEX_MODEL_PROVIDER_ID,
    "aicodemirror",
    "aicoding",
    "aigocode",
    "aihubmix",
    ...
    if source_provider_ids.is_empty() {
        crate::settings::mark_codex_third_party_history_provider_bucket_migrated(
            CodexThirdPartyHistoryProviderBucketMigration {
                ...
                target_provider_id: SWITCHFORGE_CODEX_MODEL_PROVIDER_ID.to_string(),
...
/// 迁移前 jsonl / state DB 均备份到 `~/.switchforge/backups/codex-official-history-unify-v1/`。
    ...
    config_text
        ...
        .and_then(|doc| {
            doc.get("model_provider")
                ...
                .map(|id| id.trim() == SWITCHFORGE_CODEX_MODEL_PROVIDER_ID)
        })
        .unwrap_or(false)
}
...
fn rewrite_codex_session_meta_line_for_restore(
    ...
    if payload.get("model_provider")?.as_str()? != SWITCHFORGE_CODEX_MODEL_PROVIDER_ID {
        return None;
    }
...
fn restore_codex_state_db_official_threads(
    ...
    for chunk in ids.chunks(STATE_DB_ID_CHUNK) {
        ...
        values.push(SWITCHFORGE_CODEX_MODEL_PROVIDER_ID.to_string());
...
fn is_known_cc_switch_legacy_codex_model_provider_id(provider_id: &str) -> bool {
    SWITCHFORGE_LEGACY_CODEX_MODEL_PROVIDER_IDS
        .iter()
...
fn legacy_codex_model_provider_id_from_normalized_config(config_text: &str) -> Option<String> {
    ...
    let provider_id = doc
        ...
        .and_then(|item| item.as_str())
        .map(str::trim)?;
    if provider_id != SWITCHFORGE_CODEX_MODEL_PROVIDER_ID
        && provider_id != LEGACY_SWITCHFORGE_CODEX_MODEL_PROVIDER_ID
    {
        return None;
    }
...
fn normalized_legacy_codex_provider_name(name: &str) -> Option<&'static str> {
    if is_known_cc_switch_legacy_codex_model_provider_id(name) {
        return SWITCHFORGE_LEGACY_CODEX_MODEL_PROVIDER_IDS
            .iter()
            .copied()
...
fn migrate_provider_config_template_to_custom(
    ...
    let custom_table_exists =
        config_defines_model_provider(&doc, SWITCHFORGE_CODEX_MODEL_PROVIDER_ID);
    ...
    if let Some(source_provider_id) = source_provider_id_to_move {
        ...
        model_providers[SWITCHFORGE_CODEX_MODEL_PROVIDER_ID] = provider_table;
        changed = true;
    }
    ...
    for profile_key in profile_keys {
        ...
        if references_legacy {
            profile_table.insert(
                "model_provider",
                toml_edit::value(SWITCHFORGE_CODEX_MODEL_PROVIDER_ID),
            );
            changed = true;
        }
    }
...
fn rewrite_codex_session_meta_line(
    ...
    payload.insert(
        "model_provider".to_string(),
        Value::String(SWITCHFORGE_CODEX_MODEL_PROVIDER_ID.to_string()),
    );
    serde_json::to_string(&value).ok()
}
...
"#
            ...
            Provider::with_id(
                "legacy-switchforge".to_string(),
                "Legacy SwitchForge".to_string(),
                serde_json::json!({
                    "auth": {},
                    "config": r#"model_provider = "switchforge"

[model_providers.switchforge]
name = "AIHubMix"
base_url = "https://aihubmix.example/v1"
"#
                }),
...
"#
        ...
        assert_eq!(
            source_provider_ids,
            source_ids(&["aihubmix", "switchforge", "rightcode"])
        );
        ...
        fs::write(
            ...
            concat!(
                ...
                "{\"type\":\"session_meta\",\"payload\":{\"id\":\"s3\",\"model_provider\":\"switchforge\"}}\n",
        ...
        conn.execute_batch(
            ...
            INSERT INTO threads (id, model_provider) VALUES
                ...
                ('switchforge-thread', 'switchforge'),
        ...
        assert_eq!(
            ...
            source_ids(&["legacy-switchforge", "rightcode"])
        );
        ...
        let switchforge_config: toml::Value =
            toml::from_str(&config_provider_id("legacy-switchforge"))
                .expect("parse switchforge config");
        assert_eq!(
            switchforge_config
                .get("model_provider")
                .and_then(|value| value.as_str()),
        ...
        assert!(switchforge_config
            .get("model_providers")
            .and_then(|value| value.get("switchforge"))
            .is_none());
...
"#
    ...
    #[test]
    fn collects_legacy_switchforge_provider_id_from_stored_config() {
        let db = Database::memory().expect("memory db");
        let mut provider = Provider::with_id(
            "generated-uuid".to_string(),
            ...
            serde_json::json!({
                "auth": {},
                "config": "model_provider = \"switchforge\"\n\n[model_providers.switchforge]\nname = \"AIHubMix\"\nbase_url = \"https://aihubmix.example/v1\""
            }),
            None,
        );
        provider.category = Some("aggregator".to_string());
        ...
        let ids = collect_source_model_provider_ids(&db).expect("collect ids");
        assert!(ids.contains("switchforge"));
        assert!(ids.contains("aihubmix"));
        assert!(!ids.contains("generated-uuid"));
    }
...
"#
    ...
    #[test]
    fn migrates_legacy_switchforge_provider_template_to_custom() {
        let db = Database::memory().expect("memory db");
        let provider = Provider::with_id(
            "legacy-switchforge".to_string(),
            "Legacy SwitchForge".to_string(),
...
"#
        ...
        assert_eq!(
            ...
            vec!["legacy-switchforge".to_string()]
        );

        let saved = db
            .get_provider_by_id("legacy-switchforge", "codex")
            .expect("get provider")
            .expect("provider exists");
        let config_text = saved
            .settings_config
