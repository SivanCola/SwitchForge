...
///   且不一定等于用户目录，可能导致 `.switchforge/switchforge.db` 路径变化，从而“看起来像数据丢失”。
///
/// ## 测试隔离
///
/// 为了让 Windows CI/本地测试能稳定隔离真实用户数据，可通过 `SWITCHFORGE_TEST_HOME`
/// 显式覆盖 home dir（仅用于测试/调试场景）。
pub fn get_home_dir() -> PathBuf {
    if let Ok(home) = std::env::var("SWITCHFORGE_TEST_HOME") {
...
/// 获取应用配置目录路径 (~/.switchforge)
pub fn get_app_config_dir() -> PathBuf {
    if let Some(custom) = crate::app_store::get_app_config_dir_override() {
        return custom;
    }

    // SwitchForge never probes CC Switch data implicitly. Legacy data can only
    // enter through the explicit, read-only import flow.
    get_home_dir().join(".switchforge")
}
